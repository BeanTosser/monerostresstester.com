 ...
 * "Start another withdraw" button text should be bold
 */
...
export default function Withdraw(props){  
  ...
  const changeWithdrawAddress = function(address){
    console.log("address.length: " + address.length);
      ...
      if(address.match(/[45][0-9AB][123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz]{93,93}/) &&
	 address.length === 95){
  ...
  const changeWithdrawAmount = function(amount) {
	...
	try {
	  ...
	  console.log("The entered amount in AU: " + amountInAu);
	} catch (e) {
	  console.log("The amount entered is not valid: " + e);
	...
	if(amountInAu != null) {
	  ...
	  if(amountInAu.compare(0) > 0 && amountInAu < props.availableBalance){
	    ...
	    console.log("The amount entered must be at least 1 AU and no greater than the available balance");
  ...
  const relaywithdrawTxs = async function() {
    ...
    for (let i = 0; i < withdrawInfo.length; i++){
      console.log("attempting to relay tx " + i + " of " + withdrawInfo.length);
      try {
  ...
  if(withdrawInfo.length >= 1) {
    console.log("Adding total withdraw amounts and fees")
    for (let i = 0; i < withdrawInfo.length; i++){
